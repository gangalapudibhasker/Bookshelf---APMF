...
- **Dashboard Features:**
  ...
  - **Automatic Persistence:** Saves curriculum metadata to Supabase so deployed Netlify/GitHub Pages instances and other devices show the same bookshelf records. `localStorage` is used only as a temporary offline cache.
...
Uploaded cover images can exist in Supabase Storage even when the app still shows old data. Storage only keeps the file; the bookshelf UI also needs a curriculum metadata record with the title, class, subject, cover image URL, and file attachment URL.
...
3. Confirm that:
   - the `public.curriculum` table exists with the expected columns (`title`, `subject`, `description`, `fileAttachment`, `fileName`, `classLevel`, `coverImage`, and `created_at`),
   ...
   - Realtime is enabled for `public.curriculum`.

### What the app now does
- Loads book records from `public.curriculum` and also lists files in the `book-shelf` Supabase Storage bucket, so existing uploaded Storage files can appear even before they have curriculum metadata. `localStorage` is only a temporary/offline cache if Supabase cannot be reached.
- Attempts to save every add/edit/delete metadata operation to the Supabase `curriculum` table; if metadata writes are blocked by RLS, the uploaded file still appears locally and can also appear via Storage listing once Storage read/list policies are configured.
...
The deployed app no longer renders hardcoded textbook records and filters the old demo records if they were cached or accidentally seeded. If Netlify shows an empty bookshelf, that means the browser could not read rows from `public.curriculum` or list files from `book-shelf`. Verify each Supabase row has at least `id` and `title`, verify the curriculum select policy allows anonymous reads, and verify `storage.objects` has a public read/list policy for `bucket_id = 'book-shelf'`. The app maps your actual schema as `coverImage` for covers, `fileAttachment` for file/open links, `subject` as the displayed medium/source, and `classLevel` for class text such as `Class 10`.

### Common causes when uploads do not appear in the UI
- **Only Storage changed:** A file upload succeeded, but no row was inserted/updated in `public.curriculum`. The app now lists `book-shelf` as a backup source, but curriculum metadata is still best for accurate titles/classes/file links.
- **RLS blocked metadata writes:** Storage policies may allow uploads while curriculum policies reject `insert`, `update`, or `delete` calls. If this happens, rerun `supabase/books_schema.sql`; the app no longer stops the UI update just because metadata sync was blocked.
...
### Best practices
- Treat `public.curriculum` as the source of truth and `localStorage` only as cache; do not seed or render hardcoded/demo records in production.
- Keep Storage object paths unique for replacement uploads.
- Watch the browser DevTools console and Network tab for Supabase errors, especially `401`, `403`, missing curriculum table/column errors, `NOT NULL` constraint failures, and RLS policy failures.
